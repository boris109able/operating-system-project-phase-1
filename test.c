...
void test1(void) {
	...
	printf("My Process ID is %ld. Error type is %ld.\n\n", MyProcessID, ErrorReturned);
...
void test2(void) {
	...
	//DiskID = 8;
	Sector = OurProcessID % NUMBER_LOGICAL_SECTORS;
	//Sector = 3;
	...
	strncpy(DataWritten->char_data, "123456789abcdef", 15);
    GET_TIME_OF_DAY(&Time1);
	...
	//printf("%s, %ld\n", (char*)(DataRead->char_data), (long)(DataRead->char_data));
    GET_TIME_OF_DAY(&Time2);
    printf("Time to do disk read = %ld\n", Time2 - Time1);
	//printf("%s\n", (char*)(DataRead->char_data));
...
void testD(void) {
    ...
    for (Iterations = 0; Iterations < TESTD_LOOPS; Iterations++) {
        ...
        if ((DataRead->int_data[0] != DataWritten->int_data[0])
                ...
                || (DataRead->int_data[2] != DataWritten->int_data[2])
                || (DataRead->int_data[3] != DataWritten->int_data[3])) {
            printf("ERROR in Test D, Pid = %d, Disk = %ld, Sector = %ld, Iteration = %d\n", OurProcessID, DiskID, Sector, Iterations);
            printf("Written:  %d,  %d,  %d,  %d,  %s\n", DataWritten->int_data[0],
                    DataWritten->int_data[1], DataWritten->int_data[2],
					DataWritten->int_data[3], (char*)(DataWritten->char_data));
            printf("Read:     %d,  %d,  %d,  %d,  %s\n", DataRead->int_data[0],
                    DataRead->int_data[1], DataRead->int_data[2],
					DataRead->int_data[3], (char*)(DataRead->char_data));

        } else if ((Iterations % DISPLAY_GRANULARITY_D) == 0) {
            printf("testD: SUCCESS READING  Pid = %d, DiskID =%ld, Sector = %ld\n", OurProcessID,
                    DiskID, Sector);
        }
    }   // End of for loop
...
void testX(void) {
    ...
    for (Iterations = 0; Iterations < NUMBER_OF_TEST1X_ITERATIONS;
		...
		//printf("----------------------testX %dth random sleep time is : %d\n", Iterations, RandomSleep);
        SLEEP(RandomSleep);
...
/* main(int argc, char *argv[]) {
	...
	q = createQueue();
	printQueue(q);
	pinfo = createProcessInfomation(0, 0);
	node = createQueueNode(pinfo, NODE_TYPE_TIMER);
	QueueInsert(q, node, NODE_TYPE_TIMER);
	printf("%d\n", QueueSize(q));
	printQueue(q);
	pinfo = createProcessInfomation(2, 2);
	node = createQueueNode(pinfo, NODE_TYPE_TIMER);
	QueueInsert(q, node, NODE_TYPE_TIMER);
	printf("%d\n", QueueSize(q));
	printQueue(q);
	pinfo = createProcessInfomation(1, 1);
	node = createQueueNode(pinfo, NODE_TYPE_TIMER);
	QueueInsert(q, node, NODE_TYPE_TIMER);
	printf("%d\n", QueueSize(q));
	printQueue(q);
	destroyQueue(q);
	//printQueueNode(node, NODE_TYPE_TIMER);
